...
const casesDir = positional[0] || join(root, 'test/cases')
const BATCH = 40
...
for (let i = 0; i < cases.length; i += BATCH) {
  ...
  let out = ''
  try {
    out = execFileSync(MB_BIN, ['compile'], { input: payload }).toString()
  } catch {
    // 进程非零退出：全部按失败处理
  }
...
const report = {
  schema: 1,
  suite: 'scss-diff',
  oracle: { name: 'Dart Sass JS API', package: 'sass', api: 'compileString' },
  cases_dir: casesDir,
  total: cases.length,
  passed: pass,
  failed: diff + mbErr,
